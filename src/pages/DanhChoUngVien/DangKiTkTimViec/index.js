...
function DangKiTKTimViec() {
  ...
  const handleFileChange = (e) => {
    ...
    if (file.type !== "image/png") {
      alert("Vui lòng chọn một tệp PNG.");
      fileInput.value = ""; 
      return;
    }
    ...
    if (
      ...
      try {
        const response = await fetch(
          `http://localhost:8080/account/register/participant`,
          {
            method: "POST",
            headers: {
              "Content-Type": "application/json",
  ...
  return (
    <div className={cx("formcontainerdangkitktimviec")}>
      <div className={cx("dangkitktimviec")}>
        <h2 className={cx("tieudedangkitktimviec")}>
          Đăng ký tài khoản dành cho người tìm việc
        </h2>
        <form className={cx("formdangkitktimviec")} onSubmit={handleSubmit}>
          <div className={cx("form-groupdangkitktimviec")}>
            <label className={cx("labelemaildangkitktimviec")}>
              Địa chỉ email
            </label>
            <input
              type="email"
          ...
          {verifyMessage && (
            <p className={cx("verifymessagedangkitktimviec")}>
              {verifyMessage}
            </p>
          )}

          <div className={cx("form-groupdangkitktimviec")}>
            <label className={cx("labelcodedangkitktimviec")}>
              Mã xác thực
            </label>
            <input
              type="text"
          ...
          <div className={cx("form-groupdangkitktimviec")}>
            <label className={cx("labelsdtdangkitktimviec")}>
              Số Điện Thoại
            </label>
            <input
              type="text"
          ...
          <div className={cx("form-groupdangkitktimviec")}>
            <label
              className={cx("labelavatardangkitktimviec")}
              htmlFor="avatar"
            >
              Ảnh đại diện
            </label>
            <input
              ...
              id="avatar"
              name="avatar"
              placeholder="Chọn ảnh với định dạng file/png"
              accept="image/png"
        ...
        {successMessage && (
          <p className={cx("success-messagedangkitktimviec")}>
            {successMessage}
          </p>
        )}
